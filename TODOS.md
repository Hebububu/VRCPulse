...
**What:** Vitest + @testing-library/svelte로 프론트엔드 컴포넌트 유닛 테스트 인프라 구축.
**Why:** 9+ 프론트엔드 컴포넌트/페이지가 자동화 테스트 없음. InsightCard 로케일 전환, MaintenanceBanner 표시/숨김, 필터 동작 등 모두 수동 QA 의존.
**Context:** Svelte 5 + Vite 8 환경. @testing-library/svelte 5.x가 Svelte 5 지원. vitest.config.ts에 svelte plugin 추가 필요. 우선순위: InsightCard (locale switching), MaintenanceBanner (conditional render), Dashboard (data fetching).
**Depends on:** 없음
