    ...
    async fn ready(&self, ctx: serenity::all::Context, ready: Ready) {
        ...
        match self.test_guild_id {
            ...
            None => {
                ...
                if let Err(e) = register_global(&ctx).await {
    ...
    async fn interaction_create(&self, ctx: serenity::all::Context, interaction: Interaction) {
        match interaction {
            Interaction::Command(command) => {
                ...
                super::audit::log_command(&ctx, &command);

                // Check if this guild has a pending intro (but don't send yet)
                // Use try_write() to avoid blocking the command dispatch past Discord's 3s deadline
                let pending_intro = if let Some(guild_id) = command.guild_id {
                    ...
                    if let Some(state) = data.get::<AppStateKey>() {
                        if let Ok(mut state) = state.try_write() {
                            if state.remove_pending_intro(guild_id) {
                                let locale = guild_id
                            ...
                            // Lock contended -- skip intro this time, it'll fire on next command
                            None
                        }
                    } else {
                        None
                ...
                let result = match command.data.name.as_str() {
                    "hello" => crate::hello::run(&ctx, &command).await,
                    "admin" => crate::admin::config::run(&ctx, &command).await,
                    "config" => crate::registration::run(&ctx, &command).await,
                    "report" => crate::reporting::run(&ctx, &command).await,
                    "status" => crate::status::run(&ctx, &command).await,
                    _ => Ok(()),
                };
            ...
            Interaction::Component(component) => {
                ...
                if component.data.custom_id.starts_with("config_")
                    && let Err(e) = crate::registration::handle_button(&ctx, &component).await
                {
...
/// Returns all public slash command definitions
fn all_commands() -> Vec<serenity::all::CreateCommand> {
    let mut commands = vec![
        crate::hello::register(),
        crate::registration::register(),
        crate::reporting::register(),
    ];
    commands.extend(crate::status::all());
    commands
}

/// Register global slash commands
async fn register_global(ctx: &serenity::all::Context) -> std::result::Result<(), serenity::Error> {
    let commands = serenity::all::Command::set_global_commands(&ctx.http, all_commands()).await?;
...
async fn handle_intro_button(
    ...
    match component.data.custom_id.as_str() {
        ...
        BUTTON_SET_KOREAN => {
            ...
            let repo = crate::registration::repository::GuildConfigRepository::new(db);
